<template>
  <b-container fluid class="h-100 d-flex flex-column">
      ...
      <b-sidebar id="sidebar-1" title="Global Rare eBird" visible shadow>
              ...
              <b-form-group>
                <b-input-group>
                  <template #prepend>
                    <b-dropdown class="bg-green">
                      ...
                      <b-dropdown-item @click="myLocation()"
                        ><b-icon-geo-alt class="mr-2"></b-icon-geo-alt> My location</b-dropdown-item
                      >
                    </b-dropdown>
                  </template>
                  ...
                  <b-form-input
                    ...
                    :state="(distSelected <= distMax) & (distSelected >= 0) ? null : false"
                  ></b-form-input>
                  <b-input-group-append is-text v-if="isMylocation"> km away </b-input-group-append>
                </b-input-group>
              </b-form-group>
                    ...
                    <b-list-group>
                      ...
                      >
                        <div class="d-flex w-100 justify-content-between">
                          <a
                            v-bind:href="'https://ebird.org/hotspot/' + loc.locId"
                            target="_blank"
                            title="eBird hotspot"
        ...
        <template #footer>
          <div class="d-flex bg-dark text-light align-items-center px-3 py-2 w-100 justify-content-between">
            ...
            <b-tooltip target="link-btn" triggers="click">{{ copy_status }}</b-tooltip>
    ...
    <b-modal id="modal-instruction" size="lg" ok-only title="Instruction and help">
      ...
      <p>You can share the link of the page or create a bookmark in your navigator:</p>
      ...
      <b-form-group>
        <b-form-checkbox v-model="detailSelected">
          Fetch/display information on media and comments (slow - instable)</b-form-checkbox
        >
      ...
      Use
      <a href="https://github.com/Zoziologie/global-rare-ebird/issues">Github Issues</a>.
    </b-modal>
  </b-container>
</template>
...
export default {
  ...
  methods: {
    ...
    processObs(obs, regionCode) {
      ...
      obs = obs.filter((val, index) => id.indexOf(val.speciesCode + val.subId) === index);
      let USCA = regionCode.includes("US") | regionCode.includes("CA");
      this.aba_limit = (regionCode == "US") | (regionCode == "CA") ? 3 : this.aba_limit;
      obs = obs.map((e) => {
        let o = {};
    ...
    async copyLink() {
      this.copy_status = "...";
      let str = "https://zoziologie.raphaelnussbaumer.com/global-rare-ebird/?" + this.linkUrl;
      try {
    ...
    asyncFind(query) {
      this.regionSearchFiltered = this.regionSearch.filter((o) =>
        Object.keys(o).some((k) => o[k].toLowerCase().includes(query.toLowerCase()))
      );
    },
    updateURL() {
    ...
    getIcon(loc) {
      return L.divIcon({
        ...
        iconSize: [25, 34],
        html: `
        <?xml version="1.0" encoding="UTF-8"?><svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.66 33.31"><g><path class="${
          loc.locationPrivate ? "marker-perso" : "marker-hotspot"
        }" d="M12.35,32.81c-.32,0-.8-.11-1.27-.66-2.97-3.44-5.33-6.7-7.21-9.96-1.43-2.49-2.36-4.61-2.91-6.66C.01,12.01,.54,8.71,2.52,5.74,4.22,3.21,6.6,1.55,9.6,.83c.42-.1,.86-.17,1.29-.23,.19-.03,.38-.06,.57-.09h1.67c.27,.03,.46,.06,.65,.09,.43,.06,.87,.13,1.29,.23,3.35,.83,5.92,2.79,7.64,5.83,.84,1.5,1.33,3.18,1.43,5.01,.15,2.57-.65,4.85-1.36,6.55-1.22,2.91-2.94,5.85-5.26,8.98-1.01,1.36-2.09,2.69-3.14,3.98l-.78,.97c-.45,.56-.93,.68-1.25,.68Z"/><path class="marker-outer" d="M13.13,1c.61,.1,1.23,.17,1.83,.32,3.25,.8,5.69,2.69,7.32,5.59,.83,1.48,1.27,3.09,1.37,4.79,.13,2.23-.48,4.31-1.32,6.33-1.33,3.19-3.15,6.11-5.2,8.87-1.25,1.68-2.59,3.3-3.91,4.93-.27,.33-.57,.49-.87,.49s-.61-.16-.89-.49c-2.67-3.09-5.11-6.34-7.15-9.88-1.19-2.07-2.24-4.22-2.86-6.54-.9-3.35-.43-6.5,1.5-9.38,1.63-2.44,3.91-4.01,6.78-4.7,.6-.14,1.22-.21,1.83-.32h1.59m.08-1h-1.84c-.19,.05-.37,.07-.56,.1-.43,.07-.88,.13-1.33,.24C6.35,1.1,3.87,2.82,2.11,5.46,.04,8.56-.51,11.99,.48,15.66c.56,2.09,1.5,4.25,2.96,6.78,1.89,3.29,4.27,6.57,7.26,10.03,.6,.69,1.23,.84,1.65,.84,.61,0,1.19-.31,1.64-.86l.79-.97c1.05-1.29,2.13-2.62,3.15-3.99,2.35-3.16,4.09-6.13,5.32-9.08,.73-1.75,1.55-4.1,1.4-6.77-.11-1.9-.61-3.66-1.5-5.22-1.78-3.16-4.46-5.21-7.95-6.07-.45-.11-.9-.18-1.34-.24-.19-.03-.37-.06-.56-.09h-.08Z"/></g><text class="marker-text" transform="translate(${
          loc.obs.length > 9 ? (loc.obs.length > 99 ? "0" : "4.44") : "8.39"
  ...
  computed: {
    ...
    observationsFiltered: function () {
      ...
      obsfiltered = obsfiltered.filter((x) => x.daysAgo <= parseInt(this.backSelected));
      if (this.isMylocation) {
        obsfiltered = obsfiltered.filter((x) => x.distToMe <= this.distSelected);
      }
      obsfiltered = obsfiltered.filter((x) => x.aba >= parseInt(this.aba_limit));
